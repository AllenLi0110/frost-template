...
- Reworked the visual system again after user feedback into a mobile wallet-style flow with a dark app header, horizontal workflow rail, compact summary chips, and a current-action panel.
...
- Split workflow state from scene selection after user review: the `Now` step keeps its status label without a highlighted border when the reviewer is looking at a previous scene, and the side panel now exposes a copy-vault-address action.
